...
export const update = ({ state, iteration }) => {
  const { canvas, ctx, position, motion, history } = state
  const size = 4
  ...
  */
  ...
  const flen = 70
  const fx =
    flen * Math.cos(((position[0] + position[1]) * 180 * Math.PI) / 180)
  const fy =
    flen * Math.sin(((position[0] + position[1]) * 180 * Math.PI) / 180)
  ...
  ctx.fillText(
    `dx: ${negFixed(motion[0], 2)} dy: ${negFixed(motion[1], 2)}`,
    6,
    30
  )
